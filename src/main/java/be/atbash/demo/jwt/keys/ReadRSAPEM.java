...
public class ReadRSAPEM {
    public static void main(String[] args) {
        FixedPasswordLookup lookup = new FixedPasswordLookup();
        KeyReader keyReader = new KeyReader();

        System.out.println("---------------------");
        System.out.println("-- rsa_private.pem --");
        System.out.println("---------------------");
        List<AtbashKey> atbashKeys = keyReader.readKeyResource("./rsa_private.pem", lookup);
        for (AtbashKey atbashKey : atbashKeys) {
            KeyInfo.showInfo(atbashKey);
        }

        System.out.println("--------------------");
        System.out.println("-- rsa_public.pem --");
        System.out.println("--------------------");

        atbashKeys = keyReader.readKeyResource("./rsa_public.pem");
        for (AtbashKey atbashKey : atbashKeys) {
            KeyInfo.showInfo(atbashKey);
        }
