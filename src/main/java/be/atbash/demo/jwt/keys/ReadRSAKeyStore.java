...
public class ReadRSAKeyStore {
    public static void main(String[] args) {
        KeyReader keyReader = new KeyReader();

        System.out.println("---------------------");
        System.out.println("-- rsa_private.p12 --");
        System.out.println("---------------------");
        List<AtbashKey> atbashKeys = keyReader.readKeyResource("./rsa_private.p12", new TestPassworkLookup());
        for (AtbashKey atbashKey : atbashKeys) {
            KeyInfo.showInfo(atbashKey);
        }

        System.out.println("--------------------");
        System.out.println("-- rsa_public.p12 --");
        System.out.println("--------------------");

        atbashKeys = keyReader.readKeyResource("./rsa_public.p12", new TestPassworkLookup());
        for (AtbashKey atbashKey : atbashKeys) {
            KeyInfo.showInfo(atbashKey);
        }
    ...
    public static class TestPassworkLookup implements KeyResourcePasswordLookup {
        ...
        public char[] getResourcePassword(String s) {
            return "rsaFilePassphrase".toCharArray();
        }

        @Override
        public char[] getKeyPassword(String s, String s1) {
            return "rsaKeyPassphrase".toCharArray();
        }
    }
}
