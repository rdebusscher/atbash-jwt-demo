...
public class ReadOKPPEM {
    public static void main(String[] args) {
        FixedPasswordLookup lookup = new FixedPasswordLookup();
        KeyReader keyReader = new KeyReader();

        System.out.println("---------------------");
        System.out.println("-- okp_private.pem --");
        System.out.println("---------------------");

        List<AtbashKey> atbashKeys = keyReader.readKeyResource("./okp_private.pem", lookup);
        for (AtbashKey atbashKey : atbashKeys) {
            KeyInfo.showInfo(atbashKey);
        }

        System.out.println("--------------------");
        System.out.println("-- okp_public.pem --");
        System.out.println("--------------------");

        atbashKeys = keyReader.readKeyResource("./okp_public.pem");
        for (AtbashKey atbashKey : atbashKeys) {
            KeyInfo.showInfo(atbashKey);
        }
